...
class Ball {
    ...
    boolean ballNotOnTable;

    Ball(Coord initialPosition) {
        ballNotOnTable = false;
    ...
    boolean isMovingTowardPocket(Pocket pocket){
        return Coord.distance(Coord.add(position,velocity), Coord.add(pocket.getPosition(),pocket.velocity)) <
                Coord.distance(position, pocket.getPosition());
    }
    ...
    void shoot() {
        if (isAiming()) {
            ...
            velocity = Coord.mul(Math.sqrt(30.0 * aimingVector.magnitude() / Twoballs.UPDATE_FREQUENCY),
    ...
    void move() {
        if (isMoving()) {
            ...
            ballNotOnTable = pocketCollision();
            if (ballNotOnTable){
                velocity = new Coord(0,0);
                if(!Table.cueBall.ballNotOnTable) {
                    Table.playerTurn();
                    Table.scoreAddition();
                }
            }
    ...
    public ArrayList<Coord> getballHitbox() {
        ArrayList<Coord> hitbox = new ArrayList<>(360);
    ...
    boolean pocketCollision() {
        for (Ball ball : Table.balls) {
            for (Pocket pocket : Table.pockets) {
                if (Coord.distance(pocket.getPosition(), ball.position) <= Pocket.POCKET_RADIUS && ball.isMovingTowardPocket(pocket)) {
                    return true;
                }
                if (this == Table.cueBall && Coord.distance(pocket.getPosition(), ball.position) <= Pocket.POCKET_RADIUS){
